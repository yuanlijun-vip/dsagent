...
```text
docs/未发货仅退款客户挽回自动化运行手册.md
```

本项目用于“未发货仅退款客户挽回自动化”：先生成可挽回名单，再通过千牛接待台按订单号定位买家，发送文字+图片话术，并把发送结果回写到本地 Excel 和钉钉在线表。

截至 2026-07-01，已完成 1-2 个客户的小量全自动闭环验证。批量发送前仍建议先用 `tools\prepare_send_batch.py` 锁定候选，并由操作人确认本轮发送范围。
